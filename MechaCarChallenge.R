...
data_1 <- read.csv(file='/Users/johnmiller/Desktop/MechaCar_mpg.csv',check.names=F,stringsAsFactors = F) 

#5. Perform linear regression using the lm() function
lm(mpg ~ vehicle_length + vehicle_weight + spoiler_angle + ground_clearance + AWD, data=data_1)

#6. Using the summary() function, determine the p-value and the r-squared value for the linear regression model.
summary(lm(mpg ~ vehicle_length + vehicle_weight + spoiler_angle + ground_clearance + AWD, data=data_1)) 


#Deliverable 2

#1) download suspension coil data

#2. Import and read in the Suspension_Coil.csv file as a table
mecha_coil <- read.csv(file='/Users/johnmiller/Desktop/Suspension_Coil.csv',check.names=F,stringsAsFactors = F)
